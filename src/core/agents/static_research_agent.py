...
try:
    ...
    from ...core.base.abstractions import AggregateSearchResult, ChunkSearchResult, KGSearchResult, KGEntity, KGRelationship
...
class ReasoningTool:
    ...
    parameters = {
        "type": "object",
        "properties": {
            "query": {"type": "string", "description": "The reasoning query to execute based on the conversation history."}
        },
        "required": ["query"],
    }
    def set_context(self, context: Any): pass
...
class CritiqueTool:
    ...
    parameters = {
        "type": "object",
        "properties": {
            "query": {"type": "string", "description": "A specific question to guide the critique. Can be empty."},
            "focus_areas": {"type": "array", "items": {"type": "string"}, "description": "Optional list of areas to focus the critique on."}
        },
        "required": [],
    }
    def set_context(self, context: Any): pass
...
class StaticResearchAgent:
    ...
    def _parse_llm_tool_calls(self, response_content: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            match = re.search(r"<function_calls>(.*?)</function_calls>", response_content, re.DOTALL)
            if not match:
                return []
            ...
            for invoke_elem in root.findall("invoke"):
                ...
                if name_elem is not None and name_elem.text:
                    ...
                    if params_elem is not None and params_elem.text and params_elem.text.strip():
                        try:
                            parameters = json.loads(params_elem.text)
                        except json.JSONDecodeError:
                            logger.error(f"Failed to parse JSON from <parameters> for tool {tool_name}. Content: {params_elem.text}")
    ...
    async def _execute_tool_call(self, tool_name: str, parameters: Dict[str, Any], current_config: Dict[str, Any], messages: List[Dict[str, Any]]) -> str:
        ...
        try:
            if tool_name == "search_file_knowledge":
                ...
                return tool_output_obj.llm_formatted_context or "No relevant information found by search_file_knowledge."
                ...
                return search_results_dict.get("llm_formatted_context", "No relevant file descriptions found.")
                ...
                return content_results_dict.get("llm_formatted_context", f"Could not retrieve content for document ID {doc_id}.")

            elif tool_name == "reason":
                query = parameters.get("query")
                if not query: return "Error: 'query' parameter is required for reason tool."
                ...
                if not code: return "Error: 'code' parameter is required for execute_python tool."
                timeout = parameters.get("timeout", 10)
                py_results = await self._execute_python_with_process_timeout(code, timeout)
    ...
    async def arun(self, query: str, agent_config_override: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        initial_retrieval_top_k_kg = current_config.get("initial_retrieval_top_k_kg", 10)
        ...
        if current_config.get("perform_initial_retrieval", True):
            try:
                ...
                initial_context = initial_search_results_dict.get("llm_formatted_context", "No initial search results found.")
        ...
        system_prompt_from_template = self.system_prompt_template.format(date=current_date_str)
        ...
        while iterations_count < current_max_iterations:
            ...
            try:
                llm_response = await self.llm_client.chat.completions.create(
                    ...
                    stop=["</function_calls>"] # Advise model to stop after calling functions
                )
            except Exception as e:
                logger.error(f"OpenAI API call failed: {e}", exc_info=True)
                return {"answer": "Error: LLM API call failed.", "history": messages, "error": str(e), "tools_used": executed_tools}
            ...
            if parsed_tool_calls:
                logger.info(f"LLM requested {len(parsed_tool_calls)} tool calls: {parsed_tool_calls}")
                ...
                for tool_call in parsed_tool_calls:
                    ...
                    tasks.append(self._execute_tool_call(tool_name, tool_params, current_config, messages))
...
{tool_result_str}
                    ...
                    logger.debug(f"Gathered tool result for {tool_name}: {tool_result_str[:200]}...")

                aggregated_tool_results_message = f"<function_results>\n{''.join(all_results_content)}\n</function_results>"
                
                messages.append({
                    "role": "user", 
                    "content": aggregated_tool_results_message
                })
            else: 
                # No tool calls, so this is the final answer.
                # Clean up any potential tool-related XML tags from the final response.
                final_answer = re.sub(r"<function_calls>.*</function_calls>", "", response_content, flags=re.DOTALL).replace("</function_calls>", "").strip()
                logger.info("LLM provided final answer.")
                messages.append({"role": "assistant", "content": final_answer})
                return {"answer": final_answer, "history": messages, "tools_used": executed_tools}
        ...
        return {"answer": last_llm_content, "history": messages, "warning": "Max iterations reached", "tools_used": executed_tools}
    ...
    async def knowledge_search_method(self, query: str, agent_config: Optional[Dict[str, Any]] = None) -> AggregateSearchResult:
        ...
        agg_result = AggregateSearchResult(query=query, llm_formatted_context=llm_formatted_context)
        ...
        if raw_results_dict and "sub_queries_results" in raw_results_dict:
            for sq_res in raw_results_dict["sub_queries_results"]:
                for chunk_data in sq_res.get("reranked_chunks", []):
                    ...
                    try:
                        agg_result.chunk_search_results.append(ChunkSearchResult(**{k: chunk_data.get(k) for k in ChunkSearchResult.__annotations__}))
                    except Exception as e_chunk:
                        logger.error(f"Error creating ChunkSearchResult from data: {chunk_data}, error: {e_chunk}", exc_info=True)

                for kg_data_item in sq_res.get("retrieved_kg_data", []):
                    if not isinstance(kg_data_item, dict): continue
                    try:
                        entities = [KGEntity(**e_data) for e_data in kg_data_item.get("entities", []) if isinstance(e_data, dict)]
                        relationships = [KGRelationship(**r_data) for r_data in kg_data_item.get("relationships", []) if isinstance(r_data, dict)]
                        kg_search_result_data = {**kg_data_item, "entities": entities, "relationships": relationships}
                        agg_result.graph_search_results.append(KGSearchResult(**{k: kg_search_result_data.get(k) for k in KGSearchResult.__annotations__}))
                    except Exception as e_kg:
                         logger.error(f"Error creating KGSearchResult from data: {kg_data_item}, error: {e_kg}", exc_info=True)
        return agg_result

    async def file_search_method(self, query: str, agent_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    async def content_method(self, filters: Dict, agent_config: Optional[Dict[str, Any]] = None, options: Optional[Dict] = None) -> Dict[str, Any]:
        effective_config = agent_config or self.config
        ...
        return {"error": "Document ID filter not correctly processed or missing in agent's content_method", "sub_queries_results": []}
    ...
    def _construct_tool_prompt_part(self, tools: List[Any]) -> str:
        def format_params(parameters):
            ...
            return "\n".join([f'<parameter><name>{name}</name><type>{prop.get("type", "any")}</type><description>{prop.get("description", "")}</description></parameter>' for name, prop in props.items()])
...
{joined_tool_strings}
    ...
    async def _reason(self, query: str, messages: List[Dict[str, Any]], config: Dict[str, Any]) -> str:
        """
        ...
        """
        logger.info(f"Executing reasoning tool with query: '{query}' using the agent's primary LLM.")
    ...
    async def _critique(self, query: str, focus_areas: Optional[List[str]], messages: List[Dict[str, Any]], config: Dict[str, Any]) -> str:
        """Critique the conversation history."""
        logger.info(f"Executing critique tool with query: {query} and focus areas: {focus_areas}")
        if not focus_areas:
            focus_areas = []
    ...
    async def _execute_python_with_process_timeout(self, code: str, timeout: int = 10) -> dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "success": result.returncode == 0, "timed_out": False, "timeout": timeout,
            ...
            return {
                ...
                "success": False, "timed_out": True, "timeout": timeout,
                "error": {
                    "type": "TimeoutError",
    ...
    def _format_python_results(self, results: dict[str, Any]) -> str:
        ...
        if results.get("timed_out"):
            output.append(f"⚠️ **Execution Timeout**: Code exceeded the {results.get('timeout', 10)} second limit.")
    ...
    try:
        ...
        if result.get("tools_used"):
            ...
            for i, tool_call in enumerate(result["tools_used"], 1):
                print(f"{i}. {tool_call['tool_name']}({json.dumps(tool_call['parameters'])})")
