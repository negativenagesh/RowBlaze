...
class ODTParser(AsyncParser[bytes]):
    ...
    def __init__(
        ...
        if not ODFPY_INSTALLED:
            msg = (
                "ODT parsing requires 'odfpy'. Please install it (`pip install odfpy`)."
            )
            logger.error(msg)
    ...
    def _convert_table_to_markdown(self, table_element: table.Table) -> str:
        ...
        header_texts = [
            self._extract_text_from_element(cell).strip() for cell in header_cells
        ]
        ...
        for row in rows[1:]:
            ...
            body_texts = [
                self._extract_text_from_element(cell).strip() for cell in body_cells
            ]
                        ...
                        for image in images:
                            ...
                            if href:
                                try:
                                    ...
                                    content_type = self._get_content_type_from_bytes(
                                        image_bytes
                                    )
                                    if content_type:
                                        logger.info(
                                            f"Found image '{href}'. Generating description."
                                        )
                                    ...
                                    logger.warning(
                                        f"Could not process image part '{href}': {img_e}"
                                    )
