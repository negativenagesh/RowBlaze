            ...
            for stream_path in ole.listdir(streams=True):
                try:
                    ...
                    if content_type:
                        image_count += 1
                        print(
                            f"Found image in stream {'/'.join(stream_path)} ({content_type}). Generating description."
                        )
                        description = await self._get_image_description(
                            stream_bytes, content_type
                        )
                        if description:
                            yield description
                except Exception as img_e:
                    print(
                        f"Could not read or process stream {'/'.join(stream_path)}: {img_e}"
                    )
