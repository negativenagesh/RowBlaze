                ...
                for table_data in tab.tables:
                    ...
                    for i in range(table_data.rows):
                        ...
                        for j in range(table_data.cols):
                            cell_text = table_data.cells[i][j].text if table_data.cells[i][j] else ""
                        ...
                        if table_text:
                            ...
                            if lines and len(lines) >= 2:
                                ...
                                for line in lines:
                                    ...
                                    if len(cells) <= 1:
                                        cells = [cell.strip() for cell in line.split('\t')]
                                    if len(cells) > 1:  # Only add rows that actually have multiple cells
...
class ChunkingEmbeddingPDFProcessor:
    ...
    async def _extract_knowledge_graph(
        ...
        if self.Server_type =="ARMY":
            ...
            formatted_prompt = self.graph_extraction_prompt_template.format(
                document_summary=document_summary, input=chunk_text, 
                entity_types=str([]), relation_types=str([])
            )
            messages = [
    ...
    async def _generate_document_summary(self, full_document_text: str) -> str:         
        if self.Server_type == 'ARMY':
            ...
            summary_prompt = self.summary_prompt_template.format(document=full_document_text)
            messages = [{"role": "user", "content": summary_prompt}]
            
            summary_text = await self._call_nvidia_api(
                payload_messages=messages, max_tokens=SUMMARY_MAX_TOKENS, temperature=0.3
            )
            return summary_text.strip() if summary_text else "Summary generation via NVIDIA failed."
                ...
                for attempt in range(max_retries):
                    try:
                        async with httpx.AsyncClient(timeout=60.0) as client:
                            print(f"Generating embeddings for {len(batch_texts)} texts via API: {embedding_service_url} (batch {i//batch_size + 1}, attempt {attempt + 1})")
                            
                            response = await client.post(
                                embedding_service_url, 
                                json={"texts": batch_texts, "task": "retrieval.passage"}
                            )
                            ...
                            if batch_embeddings and len(batch_embeddings) == len(batch_texts):
                                ...
                                print(f"✅ Successfully received {len(batch_embeddings)} embeddings from service.")
                                break  # Success, exit retry loop
                            else:
                                print(f"❌ Mismatch in embedding count. Expected {len(batch_texts)}, got {len(batch_embeddings)}")
                        ...
                        print(f"❌ Error calling embedding service (attempt {attempt + 1}/{max_retries}): {str(e)}")
                        if attempt == max_retries - 1:
                            # Last attempt failed, add dummy embeddings
                            dummy_embeddings = [[0.0] * 1024 for _ in batch_texts]
                            all_embeddings.extend(dummy_embeddings)
    ...
    async def _process_individual_chunk_pipeline(
        ...
        #   chunk_entities, chunk_relationships = [], []
            ...
            try:
                kg_result = await self._extract_knowledge_graph(chunk_text, contextual_summary)
                if kg_result:
                    chunk_entities, chunk_relationships = kg_result
                    print(f"KG extracted for OCR PDF chunk (Page {page_num}, Index {chunk_idx_on_page}): {len(chunk_entities)} entities, {len(chunk_relationships)} relationships.")
                else:
                    chunk_entities, chunk_relationships = [], []
                    print(f"No KG extracted for OCR PDF chunk (Page {page_num}, Index {chunk_idx_on_page}).")
            except Exception as e:
                print(f"KG extraction failed for OCR PDF chunk (Page {page_num}, Index {chunk_idx_on_page}) for '{file_name}': {e}")
                chunk_entities, chunk_relationships = [], []
            ...
            if is_docx_file:
                ...
                try:
                    ...
                    if kg_result:
                        ...
                        chunk_entities, chunk_relationships = [], []
                        print(f"No KG extracted for DOCX chunk (Page {page_num}, Index {chunk_idx_on_page}).")
                except Exception as e:
                    print(f"KG extraction failed for DOCX chunk (Page {page_num}, Index {chunk_idx_on_page}) for '{file_name}': {e}")
                    chunk_entities, chunk_relationships = [], []
            ...
            if chunk_entities:
                    ...
                    if descriptions_to_embed:
                        print(f"Generating embeddings for {len(descriptions_to_embed)} entity descriptions.")
                        try:
                            # Generate embeddings in one batch
                            description_embeddings = await self._generate_embeddings(descriptions_to_embed)
                            
                            if description_embeddings and len(description_embeddings) == len(entities_with_descriptions):
                                ...
                                for i, embedding in enumerate(description_embeddings):
                                    if embedding:  # Make sure we have a valid embedding
                                        ...
                                        chunk_entities[orig_idx]['description_embedding'] = embedding
                                ...
                                count_with_embeddings = sum(1 for e in chunk_entities if 'description_embedding' in e)
                                print(f"Entities with embeddings after assignment: {count_with_embeddings}/{len(chunk_entities)}")
                            else:
                                print(f"Mismatch between entity descriptions ({len(entities_with_descriptions)}) and embeddings ({len(description_embeddings) if description_embeddings else 0})")
                        except Exception as e:
                            print(f"Failed to generate embeddings for entity descriptions: {e}")
                            traceback.print_exc()    
            ...
            try:
                if ocr_engine == 'mistral' and MISTRAL_API_KEY and MISTRAL_SDK_AVAILABLE:
                    ...
                    async for page_text, page_num in mistral_parser.ingest(data):
                        ...
                        page_texts.append((cleaned_page_text, batch_start_page + page_num))
        ...
        if not await client.indices.exists(index=index_name):
            ...
            if is_army_mode:
                ...
                if "description_embedding" in updated_mappings["mappings"]["properties"]["metadata"]["properties"]["entities"]["properties"]:
                    updated_mappings["mappings"]["properties"]["metadata"]["properties"]["entities"]["properties"]["description_embedding"]["dims"] = 1024
            else:
...
async def example_run_file_processing(file_data: str | bytes, original_file_name: str, document_id: str, user_provided_doc_summary: str,es_client: Any, aclient_openai: Any, params: Any, config: Any):
    ...
    if not is_army_mode and not OPENAI_API_KEY:
         print("OPENAI_API_KEY is not set. LLM-dependent operations (summary, KG, enrichment, embeddings) will be skipped or use placeholders.")
    ...
    if is_army_mode:
        print("ARMY mode is active. Setting embedding dimensions to 1024 for the index.")
        dims = 1024
    else:
        print("Standard mode is active. Setting embedding dimensions to 3072 for the index.")
    ...
    print(f"Mode: {'ARMY' if is_army_mode else 'Standard'}, Embedding Dimensions: {dims}")
