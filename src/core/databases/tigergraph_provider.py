...
class TigerGraphProvider(GraphDatabaseProvider):
    ...
    async def connect(self, **kwargs) -> bool:
        ...
        self.host = kwargs.get("host") or os.getenv("TIGERGRAPH_HOST", "http://localhost:9000")
        ...
        self.password = kwargs.get("password") or os.getenv("TIGERGRAPH_PASSWORD", "tigergraph")
    ...
    async def create_node(self, label: str, properties: Dict[str, Any]) -> str:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                response = await client.post(vertex_url, json=str_properties, headers=headers)
                response.raise_for_status()
                result = response.json()
    ...
    async def create_relationship(self, from_node: str, to_node: str, 
        ...
        try:
            async with httpx.AsyncClient() as client:
                headers = {"Authorization": f"Bearer {self.token}"}
                response = await client.post(edge_url, json=str_properties, headers=headers)
                response.raise_for_status()
                result = response.json()
