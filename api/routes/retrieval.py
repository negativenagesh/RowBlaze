...
async def query_documents(
    ...
    try:
        ...
        if request.auto_chunk_sizing:
            try:
                top_k_chunks = await retriever._determine_optimal_chunk_count(request.question)
                logger.info(f"Using dynamically determined chunk count: {top_k_chunks}")
            except Exception as e:
                logger.warning(f"Error determining optimal chunk count: {e}. Using provided value.")
        ...
        return RetrievalResponse(
            answer=search_results.get("llm_formatted_context", "No formatted context generated."),
            citations=[]
        )
