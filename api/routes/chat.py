    ...
    try:
        ...
        if not chat_history.title and chat_history.messages:
            ...
            if user_messages:
                ...
                chat_history.title = (
                    (first_msg[:40] + "...") if len(first_msg) > 40 else first_msg
                )
            else:
                chat_history.title = (
                    f"Chat {chat_history.timestamp.strftime('%Y-%m-%d %H:%M')}"
                )
...
async def get_chat_history(
    session_id: str, es_client: AsyncElasticsearch = Depends(get_elasticsearch_client)
):
    """
...
async def delete_chat_history(
    ...
    try:
        ...
        # Delete document
        await es_client.delete(index=CHAT_HISTORY_INDEX, id=session_id, refresh=True)
