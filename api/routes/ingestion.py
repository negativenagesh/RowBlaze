...
async def ingest_document(
    ...
    """
    logger.info(
        f"Ingestion request received for file: {file.filename}, index: {index_name}"
    )
...
async def list_files(
    index_name: str, es_client: AsyncElasticsearch = Depends(get_elasticsearch_client)
):
    """
    ...
    try:
            ...
            try:
                ...
                response = await es_client.search(
                    ...
                    body={
                        "size": 0,
                        "aggs": {
                            "unique_files": {"terms": {"field": field, "size": 1000}}
                        },
                    },
                )
                ...
                if (
                    ...
                    and "unique_files" in response["aggregations"]
                ):
                    ...
                    if buckets:  # If we found files with this field
                        ...
                        logger.info(
                            f"Found {len(unique_files)} files using field {field}"
                        )
        ...
        if not unique_files:
            ...
            sample_response = await es_client.search(
                index=index_name, body={"size": 10, "_source": True}
            )
